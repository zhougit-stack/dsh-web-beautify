...
/** mermaid 库静态服务：loopback-only 的只读单文件（内容按进程缓存）。 */
let mermaidScript = null;
async function mermaidHandler(req, res) {
  ...
  try {
    mermaidScript ||= await readFile(new URL('./mermaid.min.js', import.meta.url));
    res.writeHead(200, {
      'content-type': 'text/javascript; charset=utf-8',
      'content-length': mermaidScript.length,
      'cache-control': 'no-store',
    });
    res.end(mermaidScript);
  } catch {
