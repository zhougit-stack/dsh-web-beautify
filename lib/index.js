...
export function createFileHandler() {
  return async (req, res) => {
    ...
    let raw;
    try {
      raw = new URL(req.url, 'http://loopback').searchParams.get('path') ?? '';
    } catch {
    ...
    const target = isAbsolute(cleaned) ? resolve(cleaned) : resolve(workspaceRoot(), cleaned);
    let info;
    try {
    ...
    const wantRaw = new URL(req.url, 'http://loopback').searchParams.get('raw') === '1';
    ...
    if (typeof ctx.inject === 'function') {
      ctx.inject(['webServer'], (httpCtx) => {
        httpCtx.effect(
          ...
          'dsh-web-beautify: file preview endpoint',
        );
      });
      return;
    }
