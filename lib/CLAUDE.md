...
- 0.5.6 切会话开关行为（用户偏好，2026-09-03 定稿）：打开（恢复）走自然过渡 = 滑入动画，健康态为 ≥3 个中间 transform 值（实测 19 帧减速曲线）且 dragFrames=0；关闭走 withoutTransition 窗 = 瞬时，健康态为 2 端点值 + dragFrames=2。实现：loadSessionState 的恢复**刻意不在**窗内（回退 0.5.5 的全窗方案），onRouteChange 关闭路径保持在窗内。用户主动开关（顶栏按钮/Esc/点击切换）始终走自然过渡。
