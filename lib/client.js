...
/* 预览开关：常驻固定位（body 直接子元素），按顶栏胶囊框测位停靠在框左侧
   （右侧只剩顶栏边距，必出框被裁），z 高于面板——面板开/关都停在同一个像素位置 */
...
`
  ...
  function ensurePanel() {
    ...
    // 开关常驻固定位：按「Session log+下载」胶囊框测位停靠在框左侧，
    ...
    const dockTopBtn = () => {
      ...
      // 胶囊框右侧到窗口缘只剩顶栏边距（~28px < 按钮 30px，必出框被裁），
      // 停靠到框左侧（镜像间隙 6px）并上移 2px；窄窗框贴左缘时退回右侧贴边
      let x = Math.round(r.left - 36)
      if (x < 8) x = Math.min(Math.round(r.right + 6), document.documentElement.clientWidth - 38)
      btn.style.left = x + 'px'
      btn.style.top = Math.round(r.top - 2) + 'px'
