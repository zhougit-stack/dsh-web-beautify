...
`
  ...
  // 切会话瞬态：临时关闭面板/会话根过渡，节后立即恢复（用户主动开关仍带动画）
  function withoutTransition(fn) {
    if (!panel.el) return fn()
    const panelCS = getComputedStyle(panel.el)
    const push = document.querySelector('.pnb-push')
    const panelTrans = panelCS.transition; const pushTrans = push ? getComputedStyle(push).transition : ''
    panel.el.style.transition = 'none'; if (push) push.style.transition = 'none'
    fn()
    requestAnimationFrame(() => requestAnimationFrame(() => {
      panel.el.style.transition = panelTrans; if (push) push.style.transition = pushTrans
    }))
  }
