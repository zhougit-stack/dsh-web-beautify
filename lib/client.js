...
.pnb-panel header{display:flex;align-items:center;gap:9px;padding:10px 54px 10px 12px;flex:0 0 auto;
...
`
  ...
  function ensurePanel() {
    ...
    const dockTopBtn = () => {
      ...
      btn.style.left = x + 'px'
      // 纵向与面板头按钮行（⧉/↗）对齐 = 头顶 + paddingTop - 2（26px 按钮与
      // 30px 开关的半径差）；面板 off-screen 时 transform 只横移，y 坐标仍有效。
      // 空面板时复制按钮 display:none 的 rect 不可用，所以按 header 测而不是按按钮测
      const hd = panel.el?.querySelector('header')
      const pt = hd ? (parseFloat(getComputedStyle(hd).paddingTop) || 10) : null
      btn.style.top = Math.round(pt != null
        ? hd.getBoundingClientRect().y + pt - 2
        : r.top + Math.max(0, r.height - 30) / 2) + 'px'
    ...
    panel.retop() // btnCopy 就绪后再校准一次，初始停靠即与复制按钮对齐
