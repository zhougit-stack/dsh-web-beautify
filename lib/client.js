...
`
  ...
  function ensurePanel() {
    ...
    const dockTopBtn = () => {
      ...
      const r = (anchor || sessBtn)?.getBoundingClientRect?.()
      if (!r || !r.width) return false
      // 胶囊框已由 CSS 视觉左移 24px，框右侧空出的位置正好放开关；
      // 右缘钳制（≥12px 边距）防窄窗再出框
  ...
  function inferSessionCwd() {
    const text = [...document.querySelectorAll('[class*="flowItem"]')]
      .slice(-200).map((e) => e.innerText || '').join('\n')
    const dirs = {}
    const re = /[A-Za-z]:[\\/](?:[^\\/:*?"<>|\r\n]+[\\/])+[^\\/:*?"<>|\r\n]*/g
    let m
  ...
  const FINGERPRINT = () => {
    ...
    const firstUser = (document.querySelector('[class*="userRow"]')?.innerText || '').trim().slice(0, 80)
