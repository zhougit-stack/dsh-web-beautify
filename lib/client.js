...
/* 预览开关：常驻固定位（body 直接子元素），按顶栏胶囊框测位停靠在框右侧
   （胶囊框本身经 CSS 左移腾出空间），z 高于面板——面板开/关都停在同一像素位置 */
.pnb-topbtn{position:fixed;z-index:1401;width:30px;height:30px;border-radius:8px;cursor:pointer;
...
.pnb-topbtn-fallback{top:12px;right:16px}
/* Session log 胶囊框视觉左移（relative 不动布局），右侧空出的位置正好
   放预览开关；样式表由本插件持有，不被 dsh 重渲染抹掉 */
[class*="sessionLogButton"]{position:relative;right:24px}
...
.pnb-panel header{display:flex;align-items:center;gap:9px;padding:15px 54px 10px 12px;flex:0 0 auto;
...
/* 复制成功/失败提示：位置由 JS 按锚点元素设定（面板层之上） */
.pnb-toast{position:fixed;z-index:1500;padding:7px 16px;border-radius:8px;
  background:rgba(36,39,46,.92);color:#fff;font-size:12.5px;white-space:nowrap;
  opacity:0;pointer-events:none;transform:translateY(4px);transition:opacity .18s,transform .18s}
.pnb-toast-show{opacity:1;transform:none}
...
/* mermaid 图表块：md 围栏 lang=mermaid 渲染成 SVG，居中放在浅框里 */
.pnb-mer{display:flex;justify-content:center;align-items:center;min-height:44px;margin:12px 0;
  padding:12px;border:1px solid rgba(15,23,42,.08);border-radius:10px;background:#fff;
  font-size:12px;color:#9aa1ab}
.pnb-mer svg{max-width:100%;height:auto}
.pnb-mer.pnb-mer-err{justify-content:flex-start;color:#b35900;white-space:pre-wrap;word-break:break-all}
...
`
  ...
  function renderMd(src) {
    ...
    for (let i = 0; i < lines.length;) {
      ...
      if ((m = FENCE.exec(L))) {
        const c = codes[+m[1]]
        // mermaid 围栏 → 占位块，setContent 后异步渲染成 SVG（renderMermaid）
        if (c.lang === 'mermaid') {
          out.push(`<div class="pnb-mer" data-mcode="${esc(c.code).replace(/"/g, '&quot;')}">⏳ mermaid 渲染中…</div>`)
          i++; continue
        }
  ...
  /* ---------------- mermaid 图表渲染 ----------------
   * 占位块（.pnb-mer[data-mcode]）→ 异步渲染成 SVG。库（~2.7MB）首次用到
   * 才从插件服务端拉一次；失败退回显示错误文本，不阻塞其余内容。 */
  const MERMAID_ENDPOINT = '/plugins/dsh-web-beautify/mermaid.min.js'
  let mermaidBooting = false

  function renderMermaid() {
    const pendingSel = () => [...panel.body.querySelectorAll('.pnb-mer:not([data-mdone])')]
    if (!panel.body || !pendingSel().length) return
    const draw = () => {
      for (const el of pendingSel()) {
        if (!el.isConnected) continue
        el.setAttribute('data-mdone', '1')
        window.mermaid.render('pnb-m' + Math.random().toString(36).slice(2, 8), el.dataset.mcode || '')
          .then(({ svg }) => { if (el.isConnected) el.innerHTML = svg })
          .catch((e) => { el.classList.add('pnb-mer-err'); el.textContent = 'mermaid 语法错误：' + String((e && e.message) || e) })
      }
    }
    if (window.mermaid) { draw(); return }
    if (mermaidBooting) return
    mermaidBooting = true
    const s = document.createElement('script')
    s.src = MERMAID_ENDPOINT
    s.onload = () => {
      mermaidBooting = false
    ...
    s.onerror = () => {
      mermaidBooting = false
      for (const el of pendingSel()) {
        el.classList.add('pnb-mer-err')
        el.textContent = 'mermaid 库加载失败（mermaid.min.js 未随插件安装）'
      }
    }
  ...
  function toast(msg, anchor) {
    let el = document.querySelector('.pnb-toast')
    if (!el) {
      el = document.createElement('div')
      el.className = 'pnb-toast'
    ...
    if (anchor) {
      ...
      y = r.bottom + 8
      if (y + h > innerHeight - 8) y = r.top - h - 8 // 贴底时翻到按钮上方
    } else {
      x = (innerWidth - w) / 2; y = innerHeight - 72
    }
    el.style.left = Math.round(x) + 'px'
    el.style.top = Math.round(y) + 'px'
    el.classList.add('pnb-toast-show')
    clearTimeout(toastTimer)
    toastTimer = setTimeout(() => el.classList.remove('pnb-toast-show'), 800)
  }
  ...
  function applyPush() {
    if (!panel.el?.classList.contains('pnb-open')) {
      // 面板关闭：零测量，只清可能残留的 transform（pnb-push 只可能标在推挤根上）
      const root = document.querySelector('.pnb-push')
      if (root) { if (root.style.transform) root.style.transform = ''; root.classList.remove('pnb-push') }
      return
    }
  ...
  function ensurePanel() {
    ...
    btn.addEventListener('click', () => {
      if (el.classList.contains('pnb-open')) { closePanel(); return }
      // 关着但已有标签：直接恢复活动标签（含滚动位），不显示空提示
      const tb = panel.tabs.find((t) => t.key === panel.activeKey) || panel.tabs[panel.tabs.length - 1]
      if (tb) { setContent(tb); return }
      showPanel({ ico: '📄', title: '预览', meta: '点击消息里的图片 / 链接 / 代码 / 文件路径打开预览' })
      panel.body.innerHTML = `<div class="pnb-loading">点击消息里的图片、网页链接、代码块或文件路径<br>即可在这里预览</div>`
    })
    // 开关常驻固定位：按「Session log+下载」胶囊框测位停靠在框右侧（框已左移让位），
    // 面板开/关都停在同一像素位置（z 1401 浮于面板之上），用户不用挪鼠标。
    // 预览开关停靠位：锚点 = dsh 顶栏「Session log」胶囊（类名稳定子串 sessionLogButton，
    // 顶栏 CSS 左移让位用的就是该选择器）。胶囊类名漂移时退文本扫描，且只扫 <header> 区域
    // （旧实现对全文档 button/div/span 做 textContent 扫描，每 4s 一次，代价过高）。
    const dockTopBtn = () => {
      let anchor = document.querySelector('[class*="sessionLogButton"]')
      if (!anchor) {
        const scope = document.querySelector('header') || document
        const matches = [...scope.querySelectorAll('button, [role="button"], div, span')]
          .filter((b) => { const t = (b.textContent || '').trim(); return /session\s*log/i.test(t) && t.length < 20 })
        const sessBtn = matches.find((m) => !matches.some((o) => o !== m && m.contains(o)))
        for (let a = sessBtn?.parentElement; a && a !== document.body; a = a.parentElement) {
          ...
          if (radii.length && Math.min(...radii) >= 8) { anchor = a; break }
          if (a.tagName === 'HEADER' || /header/i.test(a.className || '')) break
        }
      }
      const r = anchor?.getBoundingClientRect?.()
      if (!r || !r.width) return false
      // 胶囊框已由 CSS 视觉左移 24px，框右侧空出的位置正好放开关；右缘钳制（≥12px 边距）防窄窗再出框
      const x = Math.min(Math.round(r.right + 6), document.documentElement.clientWidth - 38)
      btn.style.left = x + 'px'
      // 纵向与胶囊框垂直居中（面板头 padding-top 同步 +5px 对齐这条中心线）
      btn.style.top = Math.round(r.top + Math.max(0, r.height - 30) / 2) + 'px'
      btn.classList.remove('pnb-topbtn-fallback')
      return true
    }
    panel.retop = () => {
      if (!btn.isConnected) document.body.appendChild(btn)
      if (dockTopBtn()) return
      // 胶囊框不在（主页视图/顶栏未渲染）：退右上角兜底，自愈间隔继续尝试
      btn.style.left = ''
      btn.style.top = ''
    ...
    panel.retop()
    if (btn.classList.contains('pnb-topbtn-fallback')) {
    ...
    // 稳态（按钮在且已停靠）零成本跳过；仅掉线/兜底态才重新停靠扫描
    setInterval(() => {
      if (btn.isConnected && !btn.classList.contains('pnb-topbtn-fallback')) return
      panel.retop()
    }, 4000)
    ...
    panel.btnCopy.addEventListener('click', () => {
      if (panel.copyText == null) return
      navigator.clipboard?.writeText(panel.copyText).then(
        () => toast('✓ 已复制到剪贴板', panel.btnCopy),
        () => toast('复制失败', panel.btnCopy),
      )
    })
    ...
    panel.body.addEventListener('click', (e) => {
      ...
      if (cp) {
        ...
        cp.textContent = '已复制'
        setTimeout(() => { cp.textContent = '复制' }, 800)
        return
      }
    ...
    el.querySelector('.pnb-grip').addEventListener('mousedown', (e) => {
      ...
      const move = (ev) => {
        ...
        applyPush()
        // updatePianoPos 在面板开时钉 8px，拖拽过程不需要重测——但 applyPush
        // 会改 transform，须 refresh 一下 scroller 测量确保 onresize 同步无遗漏
      }
      const up = () => {
  ...
  /** showPanel/setContent 公共头部状态（图标/标题/元信息/按钮显隐 + 开合态）。 */
  function applyHeader({ ico, title, meta = '', url = null, copyText = null }) {
  ...
  function showPanel({ ico, title, meta = '', url = null, copyText = null }) {
    ...
    applyHeader({ ico, title, meta, url, copyText })
    pushHost()
    saveSessionState()
  }
  ...
  function closeTab(i) {
    ...
    if (tb.key === panel.activeKey) {
      ...
      if (nx) setContent(nx)
      else { renderTabs(); closePanel() } // 清掉标签条 DOM，否则重开后残留僵尸标签（× 失效）
      return
    }
    renderTabs()
    saveSessionState()
  ...
  /** 会话确切 cwd：dsh 把会话工作目录作为 props.cwd 传进每个消息节点组件
   *  （fiber 上溯即得，与 loadOlder 同一条路），比消息文本推断可靠；
   *  上溯 12 跳内取第一个绝对路径形态的值。 */
  function sessionCwd() {
    ...
    for (let hop = 0; f && hop < 12; hop++, f = f.return) {
      const p = f.memoizedProps
      const c = p && (typeof p.cwd === 'string' ? p.cwd : p.ownerProps?.cwd)
      if (c && /^[A-Za-z]:[\\/]/.test(c)) return c
    }
    return null
  }

  /** 从当前会话消息文本推断会话工作目录：工具行/fileLink 里的绝对路径
   *  取最高频父目录（≥2 次才算）；fiber 拿不到 cwd 时的兜底。 */
  function inferSessionCwd() {
    const text = [...document.querySelectorAll('[class*="flowItem"]')]
      .slice(-200).map((e) => e.textContent || '').join('\n')
    const dirs = {}
    const re = /[A-Za-z]:[\\/](?:[^\\/:*?"<>|\r\n]+[\\/])+[^\\/:*?"<>|\r\n]*/g
    let m
    ...
    for (const d in dirs) if (dirs[d] > n) { n = dirs[d]; best = d }
    return n >= 2 ? best : null
  }

  const isWinAbs = (p) => /^[A-Za-z]:[\\/]/.test(p) || /^[\\/]/.test(p)

  /** 文件端点查询串；相对路径附带会话 cwd（fiber 确切值优先，文本推断兜底）。 */
  function fileQuery(p, extra = '') {
    const cwd = !isWinAbs(p) ? (sessionCwd() || inferSessionCwd()) : null
    return `path=${encodeURIComponent(p)}` + (cwd ? `&cwd=${encodeURIComponent(cwd)}` : '') + extra
  }

  /** PDF：服务端 raw 模式回流字节，iframe 用浏览器内置查看器渲染。 */
  function openPdf(p) {
    const url = `${FILE_ENDPOINT}?${fileQuery(p, '&raw=1')}`
    setContent({
  ...
  /** 点击到的路径文本 → 服务端 resolve 后的绝对路径（同一文件的不同引用
   *  形态——相对/绝对/带引号——解析缓存后命中同一个预览与同一个标签）。 */
  const resCache = new Map()

  async function openFile(rawPath) {
    const p = rawPath.trim()
    // 开关判定只看「点击时」的状态：面板开着且正显示该文件 = 收起；
    // 面板关着时重点同文件 = 重新打开（不能用点击后才写入的 lastKey 对比，否则开而立关）
    const priorKey = panel.el?.classList.contains('pnb-open') ? panel.lastKey : null
    const known = resCache.get(p)
    if (known && priorKey === 'file:' + known) { closePanel(); return }
    // 失败态（文件不存在/读取失败）没有 resolve 出的绝对路径，键 = 'file:' + 原样路径：
    // 同一不存在路径重复点击同样按开关语义收起
    if (priorKey === 'file:' + p) { closePanel(); return }
    if (/\.pdf$/i.test(p)) { openPdf(p); return }
    showPanel({ ico: '⏳', title: p })
    panel.body.innerHTML = `<div class="pnb-loading">读取文件…</div>`
    let data
    try {
      const res = await fetch(`${FILE_ENDPOINT}?${fileQuery(p)}`, { cache: 'no-store' })
      // 等待期间用户已关面板 → 丢弃迟到内容（避免面板又自己顶开）
      if (!panel.el.classList.contains('pnb-open')) return
      data = await res.json()
      if (!res.ok) { panelError(p, data.error || `读取失败（HTTP ${res.status}）`, data.path || p); panel.lastKey = 'file:' + p; return }
    } catch (error) {
      // 等待期间用户已关面板 → 丢弃迟到错误（与正常路径同判，防面板自己顶开）
      if (!panel.el?.classList.contains('pnb-open')) return
      panelError(p, `读取失败：${error}`, p); panel.lastKey = 'file:' + p; return
    }
    // 服务端 resolve 出的绝对路径作标签唯一键：同一文件永远共用一个标签
    const abs = data.path || p
    resCache.set(p, abs)
    const key = 'file:' + abs
    let meta = `${fmtSize(data.size)}${data.truncated ? ' · 已截断' : ''}`
    if (data.kind === 'image') { openImage(data.dataUrl, abs); return }
    let ico = '📄'
    let html
    if (/\.md$/i.test(abs)) {
      ico = '📝'
      ...
      html = codeHtml(data.content, langOf(abs))
    }
    // 面板开着且当前预览正是该文件（这次经另一引用形态点到）= 同样按开关语义收起
    if (priorKey === key) { closePanel(); return }
    panel.lastKey = key
    setContent({ key, ico, title: abs.split(/[\\/]/).pop(), meta, copyText: data.content, html })
  }

  /* ---------------- 琴键式对话导航（codex tick 风格） ---------------- */
  const piano = { strip: null, tip: null, keys: [], items: [], replies: [], scroller: null, column: null, older: null, raf: 0, collapsed: false, autoDone: null, autoAt: 0, lastActive: -1 }
  ...
  function classify(item) {
    ...
    if ((item.textContent || '').trim().startsWith('上下文注入')) return 'ctx'
    return 'text'
  }
  ...
  const mag = { rects: null, lastW: null, lastNear: -1 }
  let magRaf = 0; let magY = 0
  ...
  function magnify(clientY) {
    if (!mag.rects || mag.rects.length !== piano.keys.length) { measureKeys(); mag.lastW = null; mag.lastNear = -1 }
    ...
    const lastW = mag.lastW || (mag.lastW = new Array(n).fill(-1))
    let nearest = -1; let nd = Infinity
    for (let i = 0; i < n; i++) {
      const d = Math.abs(mag.rects[i].y - y)
      if (d < nd) { nd = d; nearest = i }
      // 宽度未变不写（稳态下远离光标的键恒在最小宽，跳过 style 写）
      const w = Math.round((min + (max - min) * Math.exp(-(d * d) / (2 * sigma * sigma))) * 10) / 10
      if (w !== lastW[i]) { piano.keys[i].style.width = w + 'px'; lastW[i] = w }
    }
    if (mag.lastNear !== nearest) {
      if (mag.lastNear >= 0 && piano.keys[mag.lastNear]) piano.keys[mag.lastNear].classList.remove('hover')
      if (nearest >= 0 && piano.keys[nearest]) piano.keys[nearest].classList.add('hover')
      mag.lastNear = nearest
    }
  ...
  function resetMagnify() {
    ...
    mag.rects = null; mag.lastW = null; mag.lastNear = -1
    hideTip()
  }
    ...
    if (flow) {
      ...
      for (const el of flow.items) {
        ...
        if (cur && type === 'text') {
          ...
          // 气泡只留用户消息。
          const head = (el.textContent || '').trim().slice(0, 24)
          ...
          if (!isStats && !isTool) {
            ...
            if (md && (md.textContent || '').trim()) cur.reply = md
          }
        }
      }
    }
   ...
   *  项在列内自上而下单调排列 → 二分查找，布局读取 O(log n)（旧实现逐项读到底）。 */
  function syncActive() {
    piano.raf = 0
    if (!piano.scroller) return
    updatePianoPos()
    const line = piano.scroller.getBoundingClientRect().top + piano.scroller.clientHeight * 0.45
    let lo = 0, hi = piano.items.length - 1, active = -1
    while (lo <= hi) {
      const mid = (lo + hi) >> 1
      if (piano.items[mid].getBoundingClientRect().top <= line) { active = mid; lo = mid + 1 }
      else hi = mid - 1
    }
    // 只动变化的键（滚动中 active 多数帧不变）
    if (piano.lastActive !== active) {
      if (piano.lastActive >= 0 && piano.keys[piano.lastActive]) piano.keys[piano.lastActive].classList.remove('active')
      if (active >= 0 && piano.keys[active]) piano.keys[active].classList.add('active')
      piano.lastActive = active
    }
  }
  ...
  /** 琴键条贴在「会话区入口」左缘内侧（紧邻消息区左缘、视觉属于会话区）：
   *  按 scroller（消息滚动容器）的左缘 + 14 定位——这个 scroller 跨过工作区，
   *  左缘 = 工作区右缘（典型 280）。会话列 col 与 scroller 不在同一容器层级，
   *  按 col 定位会把琴键推进消息流内被覆盖。 */
  let pianoStableLeft = 294 // scroller 默认左缘（工作区宽 280）+ 14
  function updatePianoPos() {
    if (!piano.strip) return
    if (piano.scroller?.isConnected) {
      const r = piano.scroller.getBoundingClientRect()
      if (r.left > 8) pianoStableLeft = r.left + 14 // 稳定值
    }
    piano.strip.style.left = Math.max(8, pianoStableLeft) + 'px'
  }
  ...
  function ensurePiano() {
    ...
    // mousemove 每秒可触发上百次，rAF 节流到每帧一次渐变扫掠
    strip.addEventListener('mousemove', (e) => {
      magY = e.clientY
      if (magRaf) return
      magRaf = requestAnimationFrame(() => {
        magRaf = 0
        const i = magnify(magY)
        if (i >= 0) showTip(i, magY)
      })
    })
    ...
    addEventListener('resize', () => { scheduleSync(); applyPush(); updatePianoPos(); panel.retop?.() })
  }

  /* ---------------- 会话级持久化（面板开/关 + 标签） ----------------
   * 用 URL pathname + 第一条用户消息摘要作指纹，状态按会话独立存储；
   * 切换会话 → 持久化旧指纹、查询/恢复新指纹。 */
  const SESSION_STATE = new Map()
  const FINGERPRINT = () => {
    const path = location.pathname
    const firstUser = (document.querySelector('[class*="userRow"]')?.textContent || '').trim().slice(0, 80)
    return path + '|' + firstUser
  }

  function snapshot() {
    return {
      panelOpen: panel.el?.classList.contains('pnb-open') || false,
      tabs: panel.tabs.map((t) => ({ ...t })),
      activeKey: panel.activeKey,
    }
  }
  function saveSessionState() {
    SESSION_STATE.set(FINGERPRINT(), snapshot())
  }
  // 切会话瞬态：复用宽度拖拽的 pnb-dragging 类压掉面板/会话根过渡
  // （.pnb-dragging .pnb-panel/.pnb-push{transition:none}），fn 执行完 2 帧后移除
  // （用户主动开关仍带动画）。fn 与类添加同任务同步执行，浏览器在任务结束后才
  // 算样式，故 fn 内的状态变更无过渡落地；也覆盖「pnb-push 根在 fn 内才首挂」
  // 的情况（旧实现在 fn 前查 .pnb-push，首挂时仍会走 CSS 过渡）
  function withoutTransition(fn) {
    if (!panel.el) { fn(); return }
    document.documentElement.classList.add('pnb-dragging')
    fn()
  ...
  function loadSessionState(fp) {
    const s = SESSION_STATE.get(fp)
    if (!s || !panel.el) return false
    // 恢复必须在 withoutTransition 瞬态窗内完成（与类添加同任务），
    // 否则切会话时面板会滑入/滑出（用户主动开关不受影响、保留动画）
    let hadTabs = false
    withoutTransition(() => {
      if (s.tabs?.length) {
        hadTabs = true
        panel.tabs = s.tabs
        panel.activeKey = s.activeKey
        renderTabs()
        if (s.panelOpen) {
          panel.el.classList.add('pnb-open'); panel.sidebtn.classList.add('pnb-on'); applyPush(); updatePianoPos()
          panel.body.innerHTML = ''
          setContent(s.tabs.find((t) => t.key === s.activeKey) || s.tabs[0])
        }
      } else if (s.panelOpen) {
        panel.el.classList.add('pnb-open'); panel.sidebtn.classList.add('pnb-on'); applyPush(); updatePianoPos()
        panel.body.innerHTML = `<div class="pnb-loading">点击消息里的图片、网页链接、代码块或文件路径<br>即可在这里预览</div>`
  ...
  function onClickCapture(e) {
    ...
    const t = e.target
    // 产物文件行（dsh deliverables）：按钮 title/aria-label 带绝对路径，
    // 宿主默认调外部应用打开 → 改为面板预览
    if (t.closest?.('[data-produced-files-row]')) {
      const b = t.closest('button')
      const p = b?.title || (b?.getAttribute('aria-label') || '').replace(/^打开\s*/, '')
      if (b && p && /[\\/]/.test(p)) { e.preventDefault(); e.stopPropagation(); openFile(p.trim()) }
      return
    }
  ...
  let rebuildTimer = 0
  let lastFp = ''
  // 路由级切换（pushState/replaceState/popstate）触发——比 DOM mutation 更早。
  // 切换会话点击侧栏项时 dsh 通常先 pushState 再换 DOM，这里立刻按新会话
  // 的「应有面板状态」归位（瞬态过渡关闭），不等消息流渲染。
  function onRouteChange() {
    const fp = FINGERPRINT()
    if (!fp || fp === lastFp) return
    if (lastFp) SESSION_STATE.set(lastFp, snapshot())
    lastFp = fp
    const restored = loadSessionState(fp)
    if (!restored) {
      withoutTransition(() => {
        if (panel.el?.classList.contains('pnb-open')) { panel.el.classList.remove('pnb-open'); panel.sidebtn?.classList.remove('pnb-on'); panel.lastKey = null; applyPush(); updatePianoPos() }
        panel.tabs = []
        panel.activeKey = null
        renderTabs()
      })
  ...
  function scheduleRebuild() {
    ...
    rebuildTimer = setTimeout(() => {
      ...
      // 切换会话检测：指纹（标题+首条用户消息）变化 → 持久化旧会话，尝试恢复新会话
      const fp = FINGERPRINT()
      if (fp && fp !== lastFp) onRouteChange()
  ...
  function mount() {
    ...
    // 路由切换（pushState/replaceState/popstate）触发会话指纹比对，比 DOM mutation 更早，
    // 避免「会话已加载完才闪一下恢复面板」的割裂感
    const _push = history.pushState.bind(history); history.pushState = (...a) => { _push(...a); onRouteChange() }
    const _replace = history.replaceState.bind(history); history.replaceState = (...a) => { _replace(...a); onRouteChange() }
