...
.pnb-panel header .pnb-toggle{display:inline-flex;align-items:center;justify-content:center}
...
`
  ...
  /* ---------------- 右侧预览面板 ---------------- */
  const panel = { el: null, body: null, title: null, meta: null, ico: null, btnCopy: null, btnOpen: null, sidebtn: null, openUrl: null, copyText: null, lastKey: null }
  ...
  function ensurePanel() {
    ...
    el.innerHTML = `
      ...
      <header>
        ...
        <button class="pnb-toggle" title="收起预览（Esc）">${SIDEBAR_SVG}</button>
      </header>
    ...
    panel.retop = () => {
      if (!btn.isConnected) {
        if (placeTopBtn()) btn.classList.remove('pnb-topbtn-fallback')
    ...
    if (!placeTopBtn()) {
      ...
      let tries = 0
      const retry = () => {
        if (!btn.isConnected || tries++ > 40) return
        panel.retop()
    ...
    el.querySelector('.pnb-toggle').addEventListener('click', closePanel)
