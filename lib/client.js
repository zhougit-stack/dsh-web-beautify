...
`
  ...
  /** 琴键条钉在视口固定位置（脱离宿主推挤与会话滚动的扰动）：
   *  关闭态 = 紧邻侧栏右缘；面板开/拖宽时钉在视口固定 8px 边距，
   *  不再跟随会话被推走。 */
  function updatePianoPos() {
    if (!piano.strip) return
    if (panel.el?.classList.contains('pnb-open')) {
      piano.strip.style.left = '8px' // 面板开时钉视口左缘，与拖宽/推挤解耦
      return
    }
    if (!piano.scroller) return
    const sr = piano.scroller.getBoundingClientRect()
    piano.strip.style.left = Math.max(8, sr.left + 14) + 'px'
  }
  ...
  function loadSessionState(fp) {
    ...
    if (s.tabs?.length) {
      ...
      renderTabs()
      if (s.panelOpen) { showPanel({ ico: '📄', title: '预览' }); panel.body.innerHTML = ''; setContent(s.tabs.find((t) => t.key === s.activeKey) || s.tabs[0]) }
      return true
    }
    if (s.panelOpen) { showPanel({ ico: '📄', title: '预览' }); panel.body.innerHTML = `<div class="pnb-loading">点击消息里的图片、网页链接、代码块或文件路径<br>即可在这里预览</div>` }
    return !!s.panelOpen
  }
