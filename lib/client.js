...
`
  ...
  /** 琴键条钉在「会话列左缘 - 14」处（紧邻消息区、视觉属于会话区）：
   *  - 关面板时直接读消息列几何
   *  - 开面板/拖宽时会话列被推走 → 钳制在 col 移动前的稳定位置（fallback 到 8px）
   *  - 工作区（左侧栏）不参与定位，琴键始终挂在会话区右缘附近而不是视口左缘 */
  let pianoStableLeft = 8 // 消息列左缘稳定值（开面板被推走时用）
  function updatePianoPos() {
    if (!piano.strip) return
    const col = (piano.column?.isConnected ? piano.column : findFlow()?.column)
    if (col) {
      const r = col.getBoundingClientRect()
      if (r.left > 8) pianoStableLeft = r.left + 14 // 仍在屏内 → 记录稳定值
    }
  ...
  function loadSessionState(fp) {
    ...
    const panelCS = getComputedStyle(panel.el); const push = document.querySelector('.pnb-push')
  ...
  function scheduleRebuild() {
    ...
    rebuildTimer = setTimeout(() => {
      ...
      if (fp && fp !== lastFp) {
        // 旧会话尚未显式持久化（用户没关过面板）也把当前快照写一份到旧指纹
        if (lastFp) SESSION_STATE.set(lastFp, snapshot())
        lastFp = fp
        // 新会话未登记过状态 → 关面板、清空标签；登记过则恢复
        const restored = loadSessionState(fp)
        if (!restored) {
          // 关掉动画瞬时归位（与 loadSessionState 内的处理对称）
          const panelCS = panel.el ? getComputedStyle(panel.el) : null
          const push = document.querySelector('.pnb-push')
          const panelTrans = panelCS?.transition || ''; const pushTrans = push ? getComputedStyle(push).transition : ''
          if (panel.el) panel.el.style.transition = 'none'; if (push) push.style.transition = 'none'
          requestAnimationFrame(() => requestAnimationFrame(() => {
            if (panel.el) panel.el.style.transition = panelTrans; if (push) push.style.transition = pushTrans
          }))
